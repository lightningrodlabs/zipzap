...
fn send_message(input: SendMessageInput)-> ExternResult<()> {
    send_remote_signal(Message{stream_id:input.stream_id, content:input.content}, input.agents)
}

#[hdk_extern]
fn recv_remote_signal(message: Message) -> ExternResult<()> {
    let info = call_info()?;
    let signal = Signal::Message { stream_id: message.stream_id, content: message.content, from: info.provenance };
    emit_signal(signal)
}
...
#[serde(tag = "type")]
pub enum Signal {
    Message {stream_id: String, content: String, from: AgentPubKey},
